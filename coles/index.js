    ...
    for (const loc of locations) {
      ...
      await Promise.allSettled(
        categories.map(async (categ, index) => {
                 ...
                 //  await delay(6000);
                    ...
                    while (hasProducts) {
                          ...
                          return Array.from(products).map((product) => {
                            const getPrices = (location, priceInCents, priceInCentsPerUnits, unit) => {
                              ...
                              const cleanUnit = (unit, itemName = 'N/A') => {
                                ...
                                let cleaned = unit.toUpperCase();
                              
                                if (cleaned.includes('WAS')) {
                                  cleaned = cleaned.split('WAS')[0].trim();
                                }
                                if (cleaned.includes('|')) {
                                ...
                                console.log(
                                  `\x1b[36m[Unit Cleaning]\x1b[0m Cleaned unit from \x1b[33m"${originalUnit}"\x1b[0m to \x1b[32m"${cleaned}"\x1b[0m for product \x1b[35m"${itemName}"\x1b[0m`
                                );
                      ...
                      if (productData.length > 0) {
                        ...
                        if (i===1) {
                        console.log(chalk.red(`[SKIPPED] No products found for "${extensionCategory}"`));
                        }
                        break;
                      }
                      i = i + 1;
          ...
          console.log(chalk.green('✔️ Done Category:'), chalk.yellow(category));
        })
      );
    }
