...
const matchedBaseDir = path.join(__dirname, './jsonProdToExcel');
                ...
                if (Array.isArray(jsonData)) {
                    jsonData.forEach(obj => {
                        if (obj && obj.name && obj.shop) {
                            const nswPrice = obj.prices?.find(p => p.state === "NSW")?.price || '';
                            // const vicPrice = obj.prices?.find(p => p.state === "VIC")?.price || '';
                            // const qldPrice = obj.prices?.find(p => p.state === "QLD")?.price || '';
                            // const waPrice = obj.prices?.find(p => p.state === "WA")?.price || '';
                            // const saPrice = obj.prices?.find(p => p.state === "SA")?.price || '';
                            // const tasPrice = obj.prices?.find(p => p.state === "TAS")?.price || '';
                            // const actPrice = obj.prices?.find(p => p.state === "ACT")?.price || '';
                            // const ntPrice = obj.prices?.find(p => p.state === "NT")?.price || '';
                            const entry = {
                                Name: obj.name,
...
if (fs.existsSync(matchedBaseDir)) {
    ...
    exportToExcel(woolworthsData, 'NWooly.xlsx');
    exportToExcel(colesData, 'NColes.xlsx');
} else {
